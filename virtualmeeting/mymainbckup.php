...
<body onload="fetchNotifications()">
    ...
    <div class="modal-content">
      ...
      <div class="participants">
        <div class="active-speaker participant-tile">
          ...
          <div class="label">You (Active)</div>
        </div>
      </div>
  ...
  <script>
    ...
    async function openJoinMeeting(meeting) {
      ...
      if (userId !== creatorId &&
        !meeting.participants.some(p => parseInt(p.id) === userId)) {
      ...
      try {
        ...
        const tiles = document.querySelectorAll("#joinMeetingModal .participant-tile video");
        tiles.forEach(tile => tile.srcObject = mediaStream);
      } catch (err) {
    ...
    // Open modal
    async function openTestMeeting() {
      ...
      try {
        ...
        const tiles = document.querySelectorAll(".participant-tile video");
        tiles.forEach(tile => tile.srcObject = mediaStream);
      } catch (err) {
        alert("Error: " + err);
      }
    ...
    function toggleMic() {
      ...
      showToast(micOn ? 'Microphone On' : 'Microphone Off');
    }
    ...
    async function startScreenShare() {
      try {
        ...
        const mainVideo = document.querySelector(".active-speaker video");
    ...
    async function toggleRecording() {
      ...
      if (!recording) {
        ...
        recorder.onstop = async () => {
          // Choose format
          const format = "mp4"; // default (can add custom UI to let user choose)
          ...
          // Try native save dialog (like your screenshot)
          if (window.showSaveFilePicker) {
            try {
              const fileHandle = await window.showSaveFilePicker({
                suggestedName: `meeting_recording.${format}`,
                types: [{
          ...
          a.href = url;
          a.download = `meeting_recording.${format}`;
          a.click();
        };
      }
    }
    ...
    async function openChat(meetingId) {
      ...
      if (panel.classList.contains("open")) {
        ...
        if (!panel.dataset.intervalSet) {
          setInterval(() => loadChat(meetingId), 5000);
